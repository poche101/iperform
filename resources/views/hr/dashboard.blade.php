...
@section('content')
<div class="text-2xl font-bold text-gray-900 mb-1">HR overview</div>
<div class="text-sm text-gray-500 mb-5">Org-wide insights for the {{ $cycle?->name ?? '—' }} cycle.</div>

<div class="grid grid-cols-4 gap-3 mb-5">
  @foreach([['ti-users','Total staff',$stats['total']],['ti-trending-up','Avg. score',round($stats['avg_score'] ?? 0).'%'],['ti-award','Approved',$stats['approved']],['ti-alert-triangle','With HR',$stats['with_hr']]] as [$icon,$label,$val])
      ...
      <tr class="border-b border-[#f0edf8] hover:bg-[#faf8ff]">
        ...
        <td class="py-3 px-3">
          ...
          <span class="text-[11px] font-medium px-2 py-0.5 rounded-full
            {{ $status==='approved' ? 'bg-green-100 text-green-700' : ($status==='with_hr' ? 'bg-orange-100 text-orange-700' : ($status==='submitted' ? 'bg-[#eeedfe] text-[#3C3489]' : 'bg-gray-100 text-gray-500')) }}">
            {{ ucfirst(str_replace('_',' ',$status)) }}
          </span>
        </td>
        <td class="py-3 px-3 font-bold text-[#3C3489]">{{ $a?->hr_overall ? $a->hr_overall.'%' : '—' }}</td>
        <td class="py-3 px-3 font-bold text-[#3C3489] text-lg">{{ $a?->hr_grade ?? '—' }}</td>
        <td class="py-3 px-3">
          @if($a)
